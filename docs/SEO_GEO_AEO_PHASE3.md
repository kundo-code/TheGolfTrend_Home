...
| 엔티티 일관성 | **푸터에 표시되는 이메일(`kitty2yamyam@gmail.com`)과 예약 폼 안내 문구·`SITE_CONTACT` 상수의 이메일(`edgar.meshugas@gmail.com`)이 서로 다름.** 주소도 README(`가든파이브라이프 7층 7074호`)와 실제 화면(`...7층 리빙관 7074호`)이 다름. 구조화 데이터는 실제 화면(푸터) 값을 그대로 사용했지만, **이 불일치 자체는 사업자가 확인해 하나로 정리해야 함** — 두 이메일이 각각 다른 담당자용으로 의도된 것인지 확인 필요 (임의로 통일하지 않았습니다) |
...
구조화 데이터는 실제 화면(푸터)에 보이는 값을 그대로 사용했지만, 다음은 사업자님 확인이 필요합니다(임의로 통일하지 않았습니다):
- 이메일: 푸터 `kitty2yamyam@gmail.com` vs. 예약 폼 안내문·`SITE_CONTACT` `edgar.meshugas@gmail.com`
- 주소: 실제 화면 "...가든파이브라이프 7층 **리빙관** 7074호" vs. README "...7층 7074호"(리빙관 누락)

두 이메일이 실제로 서로 다른 담당자용(예약 접수용 vs 관리자용)이라면 그대로 두어도 되지만, 검색엔진/AI에는 하나의 대표 채널로 통일된 정보가 신뢰도(entity consistency) 측면에서 유리합니다.
